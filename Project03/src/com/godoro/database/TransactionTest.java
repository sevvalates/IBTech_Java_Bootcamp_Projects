...
public class TransactionTest {
	public static void main(String[] args) throws Exception {
		...
		//transaction ı saglayan satır: (bunu eklemezsem yanlış olan işleme kadar olan işlemleri db e ekler)
		connection.setAutoCommit(false);
		
		String sql="insert into Product(productName,salesPrice) values(?,?)";
		
		PreparedStatement statement1 =connection.prepareStatement(sql);
		statement1.setString(1, "Urun 101");
		statement1.setDouble(2, 3010);
		int affected1=statement1.executeUpdate(); 
				
		PreparedStatement statement2 =connection.prepareStatement(sql);
		statement2.setString(1, "Urun 102");
		...
		statement3.setString(1, "Urun 103");
		statement3.setDouble(2, 3030);
		int affected3=statement3.executeUpdate();
		
		//commit diyene kadar commitlemez setautocommit sayesinde
		connection.commit();
